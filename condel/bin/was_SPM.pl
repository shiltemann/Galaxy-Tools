...
if (@ARGV < 2){
  die "USAGE: was.pl condel_config_dir methods_score_file\n\n";
}

my $config_dir = $ARGV[0];
my $config_file = "$config_dir/condel_SPM.conf";
my $safe_conf = 0;
...
open(MUTASS, $config{'condel.dir'}."/methdist/mutass.data");
my @mutass = <MUTASS>;
close MUTASS;

for (my $i = 0; $i < @mutass; $i++){
  if ($mutass[$i] =~ /(\S+)\s+(\S+)\s+(\S+)/){
    $mutass{'tp'}{"$1"} = $2;
    $mutass{'tn'}{"$1"} = $3;
  }
}
...
for (my $i = 0; $i < @score; $i++){
  if ($score[$i] =~ /(\S+)\s+(\S+)\s+(\S+)\s+(\S+)/){
    ...
    my $mutass_score = sprintf("%.3f", $WAS{'massess'});
    if ($mutass_score >= $config{'cutoff.HumVar.mutass'}){
      $int_score += sprintf("%.3f", $mutass_score/$config{'max.HumVar.mutass'}*(1-$mutass{'tn'}{"$mutass_score"}));
      $base += 1-$mutass{'tn'}{"$mutass_score"};
      $class{'mutass'} = 'deleterious';
    }
    else {
      $int_score += sprintf("%.3f", $mutass_score/$config{'max.HumVar.mutass'}*(1-$mutass{'tp'}{"$mutass_score"}));
      $base += 1-$mutass{'tp'}{"$mutass_score"};
      $class{'mutass'} = 'deleterious';
    }
   
    if ($base == 0){
      $int_score = 'not_computable_was';
    }
    else {
